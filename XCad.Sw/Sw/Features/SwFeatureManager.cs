...
namespace XCad.Sw.Features {
    public interface ISwFeatureManager : IXRepository<ISwFeature> {
    ...
    internal abstract class SwFeatureManager : ISwFeatureManager {
        ...
        ISwFeature IXRepository<ISwFeature>.this[string name] => this[name];

        public ISwFeature this[string name] => RepositoryHelper.Get(this, name);

        public abstract bool TryGet(string name, out ISwFeature ent);
        ...
        protected readonly EntityCache<ISwFeature> m_Cache;
        ...
        internal SwFeatureManager(SwDocument doc, SwApplication app, Context context) {
            ...
            m_Cache = new EntityCache<ISwFeature>(doc, this, f => f.Name);
        }

        public abstract void AddRange(IEnumerable<ISwFeature> feats, CancellationToken cancellationToken);

        internal void CommitCache(CancellationToken cancellationToken) => m_Cache.Commit(cancellationToken);

        public abstract IEnumerator<ISwFeature> GetEnumerator();
        ...
        public void RemoveRange(IEnumerable<ISwFeature> ents, CancellationToken cancellationToken) {
            if(Document.IsCommitted) {
                using(var viewFreeze = new UiFreeze(Document)) {
                    var disps = ents.ToSwArray<SwFeature>().Select(e => new DispatchWrapper(e.Feature)).ToArray();
        ...
        public T PreCreate<T>() where T : ISwFeature {
            if(typeof(T).IsAssignableToGenericType(typeof(ISwMacroFeature<>))) {
                var macroFeatureParamsType = typeof(T).GetArgumentsOfGenericType(typeof(ISwMacroFeature<>)).First();
                var feat = SwMacroFeature<object>.CreateSpecificInstance(null, Document, m_App, macroFeatureParamsType);
                return (T)(object)feat;
            } else {
                return RepositoryHelper.PreCreate<ISwFeature, T>(this,
        ...
        protected void CommitFeatures(IEnumerable<ISwFeature> feats, CancellationToken cancellationToken) {
            if(Document.IsCommitted) {
                using(var viewFreeze = new UiFreeze(Document)) {
    ...
    internal class SwDocumentFeatureManager : SwFeatureManager {
        ...
        public override IEnumerator<ISwFeature> GetEnumerator() {
            if(Document.IsCommitted) {
