...
namespace XCad.Sw.Documents {
    public interface ISwModelViewsCollection : IXRepository<ISwModelView> {
        ISwModelView Active { get; }
    }

    public interface ISwModelViews3DCollection : ISwModelViewsCollection {
        ISwStandardView this[StandardViewType_e type] { get; }
        new ISwNamedView this[string name] { get; }
    }

    internal class SwModelViewsCollection : ISwModelViewsCollection {
        IEnumerator IEnumerable.GetEnumerator() => GetEnumerator();
        ISwModelView IXRepository<ISwModelView>.this[string name] => this[name];
        ISwModelView ISwModelViewsCollection.Active => Active;
        ...
        public bool TryGet(string name, out ISwModelView ent) {
        ...
        public void AddRange(IEnumerable<ISwModelView> ents, CancellationToken cancellationToken) => throw new NotImplementedException();

        public IEnumerator<ISwModelView> GetEnumerator() => IterateModelViews().GetEnumerator();

        public void RemoveRange(IEnumerable<ISwModelView> ents, CancellationToken cancellationToken) => throw new NotImplementedException();

        public T PreCreate<T>() where T : ISwModelView => throw new NotImplementedException();
