...
namespace XCad.Sw.Documents {
    public interface ISwComponent : IXTransaction, IHasColor, IDimensionable, IHasName, ISwSelObject {
        ISwComponentCollection Children { get; }
        ISwDocument3D ReferencedDocument { get; set; }
        new TSelObject ConvertObject<TSelObject>(TSelObject obj)
            where TSelObject : ISwSelObject;
        IComponent2 Component { get; }
        ISwFeatureManager Features { get; }
        new ISwDimensionsCollection Dimensions { get; }

        /// <summary>
        /// Returns the cached path of the component as stored in SOLIDWORKS
        /// </summary>
        /// <remarks>This path might not correspond to actual file if component is not resolved or document is opened in view only mode. <see cref="ISwComponent.Path"/> will return the resolved path</remarks>
        ...
        ISwConfiguration ReferencedConfiguration { get; set; }
        ...
        IEditor<ISwComponent> Edit();
    }

    public interface ISwPartComponent : ISwComponent {
        new ISwPart ReferencedDocument { get; set; }
        new ISwPartConfiguration ReferencedConfiguration { get; set; }
    }

    public interface ISwAssemblyComponent : ISwComponent {
        new ISwAssembly ReferencedDocument { get; set; }
        /// <inheritdoc/>>
        new ISwAssemblyConfiguration ReferencedConfiguration { get; set; }
    }
    ...
    internal abstract class SwComponent : SwSelObject, ISwComponent {
        ISwDocument3D ISwComponent.ReferencedDocument { get => ReferencedDocument; set => ReferencedDocument = value; }
        ISwComponentCollection ISwComponent.Children => Children;
                ...
                if(IsCommitted) {
                    ...
                    if(string.IsNullOrEmpty(newPath)) {
                        ...
                        if(File.Exists(newPath)) {
                            ...
                            if(OwnerApplication.IsVersionNewerOrEqual(SwVersion_e.Sw2017)) {
            ...
            set {
                if(IsCommitted) {
                    ...
                    EditState(value, () => Component.IsVirtual, ComponentState_e.Embedded,
                        () => {
                            if(OwnerApplication.IsVersionNewerOrEqual(SwVersion_e.Sw2016)) {
                                Component.MakeVirtual2(false);
                            } else if(OwnerApplication.IsVersionNewerOrEqual(SwVersion_e.Sw2013)) {
        ...
        public ISwConfiguration ReferencedConfiguration {
            get {
                if(IsCommitted) {
                    return GetReferencedConfiguration(Component.ReferencedConfiguration);
                } else {
                    return m_Creator.CachedProperties.Get<ISwConfiguration>();
                }
            }
            set {
                if(IsCommitted) {
        ...
        protected internal abstract ISwConfiguration GetReferencedConfiguration(string confName);
        ...
        public Transform Transformation {
            get {
                if(IsCommitted) {
                    return Component.Transform2.ToXa();
                } else {
                    return m_Creator.CachedProperties.Get<Transform>();
                }
            }
            set {
                if(IsCommitted) {
                    Component.Transform2 = value.ToSw();
                } else {
                    m_Creator.CachedProperties.Set(value);
                }
            }
        ...
        internal swComponentSuppressionState_e GetSuppressionState() {
            if(RootAssembly.OwnerApplication.IsVersionNewerOrEqual(SwVersion_e.Sw2019)) {
        ...
        public abstract IEditor<ISwComponent> Edit();

        internal IComponent2 BatchComponentBuffer { get; set; }

        public ISwComponent Parent {
            get {
                var parentComp = Component.GetParent();
    ...
    internal class SwPartComponent : SwComponent, ISwPartComponent {
        ...
        ISwPartConfiguration ISwPartComponent.ReferencedConfiguration { get => (ISwPartConfiguration)base.ReferencedConfiguration; set => base.ReferencedConfiguration = value; }
        internal SwPartComponent(IComponent2 comp, SwAssembly rootAssembly, SwApplication app) : base(comp, rootAssembly, app) {
        }

        public override IEditor<ISwComponent> Edit() => new SwPartComponentEditor(RootAssembly, this);

        protected internal override ISwConfiguration GetReferencedConfiguration(string confName) => new SwPartComponentConfiguration(this, OwnerApplication, confName);
    }
    ...
    internal class SwAssemblyComponent : SwComponent, ISwAssemblyComponent {
        ...
        ISwAssemblyConfiguration ISwAssemblyComponent.ReferencedConfiguration {
            get => (ISwAssemblyConfiguration)base.ReferencedConfiguration;
            set => base.ReferencedConfiguration = value;
        }

        internal SwAssemblyComponent(IComponent2 comp, SwAssembly rootAssembly, SwApplication app) : base(comp, rootAssembly, app) {
        }

        public override IEditor<ISwComponent> Edit() => new SwAssemblyComponentEditor(RootAssembly, this);

        protected internal override ISwConfiguration GetReferencedConfiguration(string confName) => new SwAssemblyComponentConfiguration(this, OwnerApplication, confName);
    }
    ...
    internal class SwComponentFeatureManager : SwFeatureManager {
        ...
        public override void AddRange(IEnumerable<ISwFeature> feats, CancellationToken cancellationToken) {
            using(var editor = Component.Edit()) {
                CommitFeatures(feats, cancellationToken);
            }
        }

        public override IEnumerator<ISwFeature> GetEnumerator() => new ComponentFeatureEnumerator(m_Assm, GetFirstFeature(), new Context(Component));
        ...
        public override bool TryGet(string name, out ISwFeature ent) {
    ...
    internal class SwComponentBodyCollection : SwBodyCollection {
        ...
        protected override IEnumerable<IBody2> SelectSwBodies(swBodyType_e bodyType)
            => m_Comp.Component.GetBodies3((int)bodyType, out _).ToSwArray<IBody2>();
    }
    ...
    internal class SwChildComponentsCollection : SwComponentCollection {
        ...
        protected override IEnumerable<IComponent2> IterateChildren() {
            if(m_Comp.GetSuppressionState() != swComponentSuppressionState_e.swComponentSuppressed) {
                ...
                foreach(var comp in new OrderedComponentsCollection(
                    () => m_Comp.Component.GetChildren().ToSwArray<IComponent2>(),
        ...
        private void ValidateSpeedPak() {
            ...
            if(RootAssembly.OwnerApplication.IsVersionNewerOrEqual(SwVersion_e.Sw2017)) {
