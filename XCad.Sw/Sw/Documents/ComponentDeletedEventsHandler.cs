...
namespace XCad.Sw.Documents {
    internal class ComponentDeletedEventsHandler : SwModelEventsHandler<ComponentDeletedDelegate> {
        ...
        private readonly Dictionary<string, ISwComponent> m_DeletingComps;

        private readonly IXLogger m_Logger;

        internal ComponentDeletedEventsHandler(SwAssembly assm, ISwApplication app, IXLogger logger) : base(assm, app) {
            m_Assm = assm;
            m_DeletingComps = new Dictionary<string, ISwComponent>(StringComparer.CurrentCultureIgnoreCase);
            m_Logger = logger;
        }
