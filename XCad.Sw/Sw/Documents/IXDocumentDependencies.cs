...
namespace XCad.Sw.Documents {
    ...
    /// </summary>
    public interface IXDocumentDependencies : IEnumerable<ISwDocument3D> {
        /// <summary>
        ...
        void Replace(ISwDocument3D source, ISwDocument3D target);
    }
    ...
    public static class XDocumentDependenciesExtension {
        ...
        public static void ReplaceAll(this IXDocumentDependencies deps, ReplaceReferencePathProviderDelegate replacementPathProvider,
            ...
            var targetDependencies = renameMaps.Select(x => {
                var doc = app.Documents.PreCreate<ISwDocument>();
                doc.Path = x.Value;
                ...
                try {
                    foreach(var docDep in targDeps.ToArray()) {
                        ...
                        if(!string.Equals(srcDocPath, newPath, StringComparison.CurrentCultureIgnoreCase)) {
                            var newDoc = app.Documents.PreCreate<ISwDocument3D>();
                            newDoc.Path = newPath;
                            try {
        ...
        public static IEnumerable<ISwDocument3D> TryIterateAll(this IXDocumentDependencies deps, bool topLevelOnly = false,
        ...
        private static IEnumerable<ISwDocument3D> EnumerateDependencies(IXDocumentDependencies deps, bool recurse, bool isSrcReadOnly,
            bool allowCommit, bool allowReadOnly, List<string> usedPaths) {
            ISwDocument3D[] depDocs = null;
            ...
            foreach(var dep in depDocs ?? new ISwDocument3D[0]) {
        ...
        private static ISwDocument3D[] TryCommitAndGetDependencies(ISwDocument doc, bool allowReadOnly) {
            try {
                doc.Commit();
