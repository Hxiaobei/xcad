...
using XCad.Sw.UI.Commands.Attributes;
using XCad.Sw.UI.Commands.Exceptions;
using XCad.Sw.UI.Commands.Toolkit.Enums;
using XCad.Sw.UI.Commands.Toolkit.Structures;
using XCad.Sw.Utils;
using XCad.UI;
    ...
    internal class SwCommandManager : ISwCommandManager {
        ...
        public IXCommandGroup AddContextMenu(ContextMenuCommandGroupSpec cmdBar) {
            ...
            if(cmdBar.Owner != null) {
                ...
                if(cmdBar is ContextMenuEnumCommandGroupSpec spec) {
                    var swCtxMenInfoAtt = spec
        ...
        internal void HandleCommandClick(string cmdId) {
            ...
            if(m_Commands.TryGetValue(cmdId, out CommandInfo cmd)) {
        ...
        private IImageCollection CreateMainIcon(CommandGroupSpec cmdBar, IIconsCreator iconsConv) {
            var mainIcon = cmdBar.Icon ?? Defaults.Icon;
        ...
        private bool TryClearTab(string tabName, swDocumentTypes_e docType, ref CommandTab cmdTab) {
            foreach(var cmdTabBox in cmdTab.CommandTabBoxes().ToSwArray<CommandTabBox>()) {
        ...
        private bool IsCommandTabContainingGroups(ICommandTab cmdTab, TabCommandGroupInfo[] tabGroups) {
            var tabBoxes = cmdTab.CommandTabBoxes().ToSwArray<ICommandTabBox>();

            if(tabBoxes.Length == tabGroups.Length) {
                for(int i = 0; i < tabBoxes.Length; i++) {
                    if(!IsCommandTabBoxMatchingGroup(tabBoxes[i], tabGroups[i])) {
                        return false;
                    }
                }
        ...
        private bool IsCommandTabBoxMatchingGroup(ICommandTabBox cmdTabBox, TabCommandGroupInfo groupInfo) {
            cmdTabBox.GetCommands(out object existingCmds, out object existingTextStyles);
