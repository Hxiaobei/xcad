...
using XCad.UI.TaskPane.Delegates;
...
namespace XCad.Sw.UI {
    ...
    internal class SwTaskPane<TControl> : ISwTaskPane<TControl>, IAutoDisposable {
        ...
        private int OnTaskPaneToolbarButtonClicked(int buttonIndex) {
            m_Logger.Log($"Task pane button clicked: {buttonIndex}", LoggerMessageSeverity_e.Debug);

            if(m_Spec.Buttons?.Length > buttonIndex) {
                ButtonClick?.Invoke(m_Spec.Buttons[buttonIndex]);
            } else {
                m_Logger.Log($"Invalid task pane button id is clicked: {buttonIndex}", LoggerMessageSeverity_e.Error);
        ...
        private int OnTaskPaneDestroyNotify() {
            m_Logger.Log("Destroying task pane", LoggerMessageSeverity_e.Debug);
