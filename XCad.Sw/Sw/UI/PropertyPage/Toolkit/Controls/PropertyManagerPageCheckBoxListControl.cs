        ...
        protected override ItemsControlItem[] CreateEnumItems(Type enumType) {
            if(enumType.IsEnum
                ...
                foreach(Enum item in items) {
                    // 检查是否可见
                    var browsableAttr = item.TryGetAttribute<BrowsableAttribute>();
                    var visible = browsableAttr?.Browsable ?? true;

                    if(visible) {
                        var affectedFlags = flags.Where(item.HasFlag).ToArray();

                        // 获取显示名称
                        var displayNameAttr = item.TryGetAttribute<DisplayNameAttribute>();
                        var name = displayNameAttr?.DisplayName ?? item.ToString();

                        // 获取描述信息
                        var descAttr = item.TryGetAttribute<DescriptionAttribute>();
