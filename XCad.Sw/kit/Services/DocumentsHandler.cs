...
using XCad.Sw.Documents.Attributes;
...
namespace XCad.kit.Services {
    ...
    public class DocumentsHandler : IDisposable {
        ...
        private readonly Dictionary<ISwDocument, List<IDocumentHandler>> m_DocsMap;

        private readonly IXLogger m_Logger;

        public DocumentsHandler(ISwApplication app, IXLogger logger) {
            m_App = app;
            m_Logger = logger;

            m_Handlers = new List<DocumentHandlerInfo>();
            m_DocsMap = new Dictionary<ISwDocument, List<IDocumentHandler>>(new XObjectEqualityComparer<ISwDocument>());
        }
        ...
        public void RegisterHandler<THandler>(Func<THandler> handlerFact) where THandler : IDocumentHandler {
            ...
            Type[] filters = type.TryGetAttribute<DocumentHandlerFilterAttribute>()?.Filters;
        ...
        private void OnDocumentLoaded(ISwDocument doc)
            => TryInitHandlers(doc);
        ...
        private void TryInitHandlers(ISwDocument doc) {
        ...
        private void TryReleaseHandlers(ISwDocument doc) {
        ...
        private void CreateHandler(ISwDocument doc, DocumentHandlerInfo handlerInfo, List<IDocumentHandler> handlersList) {
        ...
        private void OnDocumentDestroyed(ISwDocument doc)
