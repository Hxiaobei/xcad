...
using XCad.kit.PageBuilder.Exceptions;
using XCad.Sw;
using XCad.Sw.Base;
            ...
            foreach(var prp in type.GetProperties()) {
                ...
                if(!processedTypes.Contains(prpType)) {
                    if(!prpType.IsPrimitive
                        ...
                        && !typeof(ISwObject).IsAssignableFrom(prpType)) {
                    ...
                    m_Logger.Log($"Type '{prpType.FullName}' is skipped as it was already processed while extracting metadata", LoggerMessageSeverity_e.Debug);
                }
            }
        }
    }
