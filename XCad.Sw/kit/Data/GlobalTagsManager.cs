...
namespace XCad.kit.Data {
    ...
    public class GlobalTagsRegistry : IDisposable {
        private class DocumentObjectTags : Dictionary<ISwObject, LocalTagsManager> {
            internal DocumentObjectTags() : base(new XObjectEqualityComparer<ISwObject>()) {
            }
        }

        private readonly Dictionary<ISwDocument, DocumentObjectTags> m_Tags;

        public GlobalTagsRegistry() {
            m_Tags = new Dictionary<ISwDocument, DocumentObjectTags>(new XObjectEqualityComparer<ISwDocument>());
        }

        public int Count => m_Tags.Count;

        public bool IsEmpty(ISwObject owner) {
        ...
        public bool Contains(ISwObject owner, string name) {
        ...
        public T Get<T>(ISwObject owner, string name) {
            if(TryGetLocalTagsManager(owner, out var tagsMgr, out _)) {
        ...
        public T Pop<T>(ISwObject owner, string name) {
        ...
        public void Put<T>(ISwObject owner, string name, T value) {
        ...
        private void OnOwnerDocumentDestroyed(ISwDocument doc) {
        ...
        private bool TryGetLocalTagsManager(ISwObject owner, out LocalTagsManager tagsMgr, out DocumentObjectTags docTags) {
    ...
    /// </summary>
    /// <remarks>Use this service instead of <see cref="LocalTagsManager"/> when instance of <see cref="ISwObject"/>
    /// is not guaranteed to be the same for the native (underline) object</remarks>
    public class GlobalTagsManager : ITagsManager {
        private readonly ISwObject m_Owner;
        private readonly GlobalTagsRegistry m_Tags;

        public GlobalTagsManager(ISwObject owner, GlobalTagsRegistry globalTagsReg) {
            m_Owner = owner;
