...
namespace XCad.kit.Utils {
    ...
    public static class PopupHelper {
        ...
        public static Vec3d CalculateLocation(IntPtr parentWnd, PopupDock_e dock, bool scaleDpi, double width, double height, Thickness padding) {
            ...
            switch(dock) {
                case PopupDock_e.Center:
                    return new Vec3d(left + wndWidth / 2 - width / 2, top + wndHeight / 2 - height / 2, 0);

                case PopupDock_e.TopRight:
                    return new Vec3d(left + wndWidth - width - padding.Right, top + padding.Top, 0);

                case PopupDock_e.TopLeft:
                    return new Vec3d(left + padding.Left, top + padding.Top, 0);

                case PopupDock_e.BottomRight:
                    return new Vec3d(left + wndWidth - width - padding.Right, top + wndHeight - height - padding.Bottom, 0);

                case PopupDock_e.BottomLeft:
                    return new Vec3d(left + padding.Left, top + wndHeight - height - padding.Bottom, 0);
